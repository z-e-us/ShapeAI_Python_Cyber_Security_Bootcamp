# -*- coding: utf-8 -*-
"""Weather.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/16EK4yvZVNtu1aYoWGLfd9zr1dHEALjnf
"""

import requests
from datetime import datetime

api_key = 'a4d7df52094bdb3585fb010a549f7377'
location = input("Enter the city name: ")
complete_api_link = "https://api.openweathermap.org/data/2.5/weather?q="+location+"&appid="+api_key
api_link = requests.get(complete_api_link)
api_data = api_link.json()
temp_city = ((api_data['main']['temp']) - 273.15)
weather_desc = api_data['weather'][0]['description']
hmdt = api_data['main']['humidity']
wind_spd = api_data['wind']['speed']
date_time = datetime.now().strftime("%d %b %Y| %I:%M:%S %p")

print ("-----------------------------------------------------------------")
print ("Weather Stats for - {} || {} ".format(location.upper(), date_time))
print ("-----------------------------------------------------------------")
print ("Current temperature is: {:.2f} deg C".format(temp_city))
print ("Curretn weather desc :",weather_desc)
print ("Current Humidity     :",hmdt, '%')
print ("Current wind speed   :",wind_spd,'kmph')

with open('weather.txt','w')as out:
  out.write('-----------------------------------------------------------------\n');
  out.write('Weather Stats for - {} || {} '.format(location.upper(), date_time));out.write('\n');
  out.write('-----------------------------------------------------------------\n');
  out.write('Current temperature is: {:.2f} deg C'.format(temp_city));
  out.write('\n');
  out.write('Curretn weather desc :{}'.format(weather_desc));
  out.write('\n');
  out.write('Current Humidity     :{}'.format(hmdt));
  out.write('%');
  out.write('\n');
  out.write('Current wind speed   :{}'.format(wind_spd));
  out.write('kmph');